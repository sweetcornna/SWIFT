...
| Training smoke mistaken for convergence | A short CPU PPO smoke can prove the training loop works without proving the policy has converged. | Report smoke metrics separately from tuning/convergence claims; preserve history JSONL and checkpoints for audit/resume; use deterministic tuning and later longer training runs for performance evidence. |
